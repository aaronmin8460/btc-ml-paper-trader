...
def format_diagnosis_report(report: dict[str, Any], settings: Settings) -> str:
    ...
    lines = [
        ...
        f"sell_quality_label distribution: {_format_distribution(summary['sell_quality_label_distribution'])}",
        ...
        f"sell positive count: {summary['sell_positive_label_count']}",
        "",
    ...
    lines.extend(
        [
            "",
            "estimated required exit return from current fee/slippage/spread/min-profit settings: "
            f"{summary['estimated_required_exit_return_pct']:.6f} "
            f"({_format_pct(summary['estimated_required_exit_return_pct'])})",
        ]
    )
...
def _format_comparison_table(rows: list[dict[str, Any]]) -> str:
    headers = ["config", "trainable_rows", "buy_1", "buy_0", "buy_1_pct", "sell_1", "sell_0", "sell_1_pct"]
    formatted_rows = [
        {
            **row,
            "buy_1_pct": _format_pct(float(row["buy_1_pct"])),
            "sell_1_pct": _format_pct(float(row["sell_1_pct"])),
        }
        for row in rows
    ]
    widths = {
