...
def format_training_report(result: dict[str, Any], settings) -> str:
    ...
    lines = [
        ...
        f"exit/sell positive label count: {metrics.get('exit_positive_label_count', metrics.get('sell_positive_label_count', 'unknown'))}",
        f"buy/sell imbalance ratio: {_format_number(metrics.get('buy_sell_imbalance_ratio'))}",
        f"training label required exit return pct: {_format_pct(metrics.get('training_estimated_required_exit_return_pct'))}",
        f"conservative promotion required exit return pct: {_format_pct(metrics.get('conservative_promotion_estimated_required_exit_return_pct'))}",
        "top NaN columns:",
