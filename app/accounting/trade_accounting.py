...
def record_filled_order_trade(repo: Repository, order: Order | None, settings: Settings) -> Trade | None:
    ...
    matched_qty, exit_price, pnl = realized
    if matched_qty <= 0:
        return None
    existing = _existing_realized_trade(repo, order=order, qty=matched_qty, price=exit_price, pnl=pnl)
...
def _realized_pnl_for_sell(
    ...
    pnl_for_target = 0.0
    exit_price_for_target = _effective_sell_price(sell_execution.price, settings)
    ...
    for order in _filled_orders_through(repo, sell_execution.order):
        ...
        if execution.side == "buy":
            open_lots.append(
                OpenLot(
                    qty=execution.qty,
                    effective_price=_effective_buy_price(execution.price, settings),
        ...
        effective_sell_price = _effective_sell_price(execution.price, settings)
    ...
    if matched_qty_for_target <= 0:
        return None
    return matched_qty_for_target, exit_price_for_target, pnl_for_target
...
def _execution_from_order(order: Order, settings: Settings) -> Execution | None:
    ...
    return Execution(order=order, side=side, qty=qty, price=price, fee=max(0.0, fee), fee_from_broker=fee_from_broker)


def _is_filled_order(order: Order) -> bool:
    return order.symbol == ALLOWED_SYMBOL and str(order.status or "").strip().lower() == "filled"


def _effective_buy_price(price: float, settings: Settings) -> float:
    return price * (1 + settings.paper_slippage_bps / 10_000)


def _effective_sell_price(price: float, settings: Settings) -> float:
    return price * (1 - settings.paper_slippage_bps / 10_000)
...
def _fee_from_raw(raw: dict[str, Any]) -> tuple[float | None, bool]:
    for key in ("fee", "fees", "commission", "commission_amount", "filled_fee"):
        if key not in raw:
            continue
